    ...
    <div class="projectMedia">
        ...
        <div class="projectMedia__list" v-if="props.readOnly === true">
            ...
            <div v-for="(file, index) in filteredFiles" :key="index" class="">
                <div class="" v-if="file.pictureUrl == null || ''"></div>
                <div v-else class="projectMedia__item">
                    <div style="gap: 10px" class="d-flex align-center">
                        ...
                        <p style="color: #9e9e9e" class="txt-cap1 cursor-pointer">
                            <a :href="sanitizeUrl(file.pictureUrl)" target="_blank" rel="noopener noreferrer">перейти</a>
                        </p>
                    </div>
        ...
        <div class="projectMedia__list" v-if="props.readOnly === false">
            <div v-for="(file, index) in filteredFiles" :key="index" class="">
                <div v-if="file.pictureUrl == null || ''"></div>
                <div v-else class="projectMedia__item">
                    <div style="gap: 10px" class="d-flex align-center">
                        <img src="../../assets/icons/media/video.svg" />
                        <p style="color: #9e9e9e" class="txt-cap1 cursor-pointer">
            ...
            <div v-if="filteredFiles.length < 6" @click="mediaState.open()" class="projectMedia__item projectMedia__item--adder cursor-pointer">
                <v-progress-circular v-if="loading" width="2" class="loading mx-auto text-center mt-2" color="active" indeterminate></v-progress-circular>
                ...
                <vue-bottom-sheet ref="mediaState">
                    <div class="mx-5 mb-10">
                        <p class="txt-body1 mb-12">
                            Добавьте ссылку на видео-хостинг или загрузите файл в удобное облако и прикрепите ссылку на него.
                        </p>
                        <UiInput v-model="link" id="linkInput" label="Ссылка*" />
                        <UiInput label="Описание ссылки*" class="my-8" />
                        <div @click="mediaState.close()">
                            <UiButton @click="submitLink" bgColor="blue">Добавить</UiButton>
                        </div>
                    </div>
                </vue-bottom-sheet>
            </div>
...
import { addProjectFile, getProjectByID, deleteProjectFile } from '~/API/ways/project'
import { addTalentFile, getTalentByID } from '~/API/ways/talent'
...
const submitLink = async () => {
    ...
    setTimeout(async () => {
        try {
            if (props.isTalent) {
                await addTalentFile(Number(router.params.ID), uploadedFile.value, link.value);
            } else {
                await addProjectFile(link.value, Number(router.params.ID));
            }
...
const fetchData = async () => {
    if (props.isTalent) {
        await getTalentByID(router.params.ID).then((response) => {
            try {
