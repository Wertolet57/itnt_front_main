const projectSkills = {
    "TECHNOLOGY_3D": "3D технологии",
    "ACCOUNTING": "Бухгалтерия",
    "ADVANCED_MATERIALS": "Перспективные материалы",
    "ADVERTISING": "Реклама",
    "AEROSPACE": "Авиационная промышленность",
    "AGRICULTURE_TECH": "Сельское хозяйство и технологии",
    "ANALYTICS": "Аналитика",
    "ANIMALS": "Животные",
    "AR_VR": "Дополненная и виртуальная реальность",
    "ART": "Искусство",
    "ARTIFICIAL_INTELLIGENCE": "Искусственный интеллект",
    "ASSET_MANAGEMENT": "Управление активами",
    "AUCTIONS": "Аукционы",
    "AUDIO": "Аудио",
    "AUTOMOTIVE": "Автомобильная промышленность",
    "AUTONOMOUS_VEHICLES": "Автономные транспортные средства",
    "BANKING": "Банковское дело",
    "BEAUTY": "Красота",
    "BIOTECHNOLOGY": "Биотехнологии",
    "BOOKS": "Книги",
    "CHEMICAL": "Химическая промышленность",
    "CHILDREN": "Дети",
    "CLEAN_RENEWABLE_ENERGY": "Чистая и возобновляемая энергия",
    "CLEAN_TECH": "Чистые технологии",
    "CLEANING": "Уборка",
    "CLINICAL_TRIALS": "Клинические исследования",
    "CLOUD_INFRASTRUCTURE": "Облачная инфраструктура",
    "COLLABORATION": "Сотрудничество",
    "COMMUNICATIONS_INFRASTRUCTURE": "Инфраструктура коммуникаций",
    "COMMUNITIES": "Сообщества",
    "COMPLIANC": "Соответствие",
    "COMPUTER_VISION": "Компьютерное зрение",
    "CONSTRUCTION": "Строительство",
    "CONSULTING": "Консультирование",
    "CONSUMER": "Потребительский рынок",
    "CONSUMER_ELECTRONICS": "Потребительская электроника",
    "CONSUMER_GOODS": "Потребительские товары",
    "CONTENT": "Контент",
    "CREATOR_ECONOMY": "Экономика создателей",
    "CRM_SYSTEMS": "CRM-системы",
    "CRYPTO": "Крипто",
    "CUSTOMER_SERVICE": "Обслуживание клиентов",
    "CYBER_SECURITY": "Кибербезопасность",
    "DATA": "Данные",
    "DATING": "Знакомства",
    "DELIVERY": "Доставка",
    "DESIGN": "Дизайн",
    "DEVELOPER_API": "API для разработчиков",
    "DEVELOPER_TOOLS": "Инструменты для разработчиков",
    "DRONES": "Дроны",
    "E_COMMERCE": "Электронная коммерция",
    "EDUCATION": "Образование",
    "EGAMES": "Компьютерные игры",
    "ELDER_CARE": "Уход за пожилыми",
    "ELECTRIC_VEHICLE": "Электромобили",
    "ELECTRONICS": "Электроника",
    "EMPLOYEE_BENEFITS": "Льготы для сотрудников",
    "ENERGY": "Энергетика",
    "ENTERPRISE_SOFTWARE": "Корпоративное ПО",
    "ENTERTAINMENT_MEDIA": "Развлечения и СМИ",
    "EVENTS": "Мероприятия",
    "FAMILY": "Семья",
    "FARMING": "Фермерство",
    "FASHION": "Мода",
    "FILM": "Кино",
    "FINTECH": "Финтех",
    "FITNESS": "Фитнес",
    "FOOD_BEVERAGE": "Еда и напитки",
    "FOOD_DELIVERY": "Доставка еды",
    "FOOD_PROCESSING": "Переработка продуктов питания",
    "FORESTRY": "Лесное хозяйство",
    "FUEL": "Топливо",
    "FUNDING_PLATFORM": "Платформа для финансирования",
    "GENETICS": "Генетика",
    "GEOSPATIAL": "Геопространственные технологии",
    "GIFTS_COUPONS": "Подарки и купоны",
    "GOVERNMENT": "Государство",
    "GREENTECH": "Зеленые технологии",
    "HARDWARE": "Аппаратное обеспечение",
    "HEALTH_CARE": "Здравоохранение",
    "HOME_GARDEN": "Дом и сад",
    "HOSPITALITY": "Гостиничный бизнес",
    "HR_TECH": "Технологии для HR",
    "HUMAN_COMPUTER_INTERACTION": "Взаимодействие человека и компьютера",
    "IDENTITY_MANAGEMENT": "Управление идентификацией",
    "INDUSTRIAL": "Промышленность",
    "INFORMATION_SERVICES": "Информационные услуги",
    "INSURANCE": "Страхование",
    "INTERNET_OF_THINGS": "Интернет вещей",
    "IT_INFRASTRUCTURE": "ИТ-инфраструктура",
    "LEGAL": "Юридические услуги",
    "LENDING_CREDIT": "Кредитование и займы",
    "LIFE_SCIENCE": "Науки о жизни",
    "LIFESTYLE": "Образ жизни",
    "LOGISTICS": "Логистика",
    "MACHINE_LEARNING": "Машинное обучение",
    "MANUFACTURING": "Производство",
    "MARINE_TECHNOLOGY": "Морские технологии",
    "MARKETING": "Маркетинг",
    "MARKETPLACES": "Маркетплейсы",
    "MEDICAL": "Медицина",
    "MEDICAL_DEVICE": "Медицинские устройства",
    "MESSAGING": "Сообщения",
    "MUSIC": "Музыка",
    "NANOTECHNOLOGY": "Нанотехнологии",
    "NATURAL_LANGUAGE_PROCESSING": "Обработка естественного языка",
    "NATURAL_RESOURCES": "Природные ресурсы",
    "NUTRITION": "Питание",
    "ONLINE_PORTALS": "Онлайн-порталы",
    "PAYMENTS": "Платежи",
    "PERSONAL_FINANCE": "Личные финансы",
    "PERSONAL_HEALTH": "Личное здоровье",
    "PHARMACEUTICAL": "Фармацевтика",
    "PHOTOGRAPHY": "Фотография",
    "PRIVACY": "Конфиденциальность",
    "PRODUCTIVITY_TOOLS": "Инструменты для повышения продуктивности",
    "PROFESSIONAL_SERVICES": "Профессиональные услуги",
    "PROJECT_MANAGEMENT": "Управление проектами",
    "PROPERTY": "Недвижимость",
    "QUANTUM_COMPUTING": "Квантовые вычисления",
    "REAL_ESTATE": "Недвижимость",
    "REAL_TIME": "Реальное время",
    "RECOGNITION": "Распознавание",
    "RECRUITING": "Рекрутинг",
    "RESTAURANTS": "Рестораны",
    "RETAIL": "Розничная торговля",
    "RISK_MANAGEMENT": "Управление рисками",
    "ROBOTICS": "Робототехника",
    "SAAS": "ПО как услуга",
    "SALES": "Продажи",
    "SEARCH_ENGINES": "Поисковые системы",
    "SECURITY": "Безопасность",
    "SEMICONDUCTOR": "Полупроводники",
    "SEO": "Поисковая оптимизация",
    "SERVICE_INDUSTRY": "Сфера услуг",
    "SEX_INDUSTRY": "Секс-индустрия",
    "SHARING_ECONOMY": "Экономика совместного потребления",
    "SHOPPING": "Шоппинг",
    "SMART_HOME_CITIES": "Умный дом и умные города",
    "SMB": "Малый и средний бизнес",
    "SOCIAL": "Социальная сфера",
    "SOCIAL_NETWORKS": "Социальные сети",
    "SOFTWARE_ENGINEERING": "Программная инженерия",
    "SOLAR": "Солнечная энергия",
    "SPACE_TRAVEL": "Космические путешествия",
    "SPORTS": "Спорт",
    "SUPPLY_CHAIN_MANAGEMENT": "Управление цепочками поставок",
    "SUSTAINABILITY": "Устойчивое развитие",
    "TELECOMMUNICATIONS": "Телекоммуникации",
    "TEST_MEASUREMENT": "Тестирование и измерения",
    "THERAPEUTICS": "Терапевтические средства",
    "TOURISM": "Туризм",
    "TRANSPORTATION": "Транспорт",
    "TRAVEL": "Путешествия",
    "TV_RADIO": "ТВ и радио",
    "VIDEO": "Видео",
    "WEARABLES": "Носимые устройства",
    "WEB3": "Веб 3.0",
    "WEB_DEVELOPMENT_APPS": "Веб-разработка и приложения",
    "WELLNESS": "Благополучие",
    "WINE_SPIRITS": "Вино и спиртные напитки"
}
export default projectSkills;
