        ...
        <v-row class="container pa-0 ma-0" justify="center" align="center">
            ...
            <div v-if="pageStep === 3" class="button-container text-center">
                ...
                <div style="
                    ...
                    ">
                    <v-otp-input ref="otpInput" input-classes="otp-input" separator="" :num-inputs="4"
                        v-model:value="pinCode" :should-auto-focus="true" input-type="number" @on-complete="sendPinCode"
                        :class="{
                'otp-input--has-value': pinCode.length > 0,
                'red-bottom-border': codeError,
                'blue-bottom-border': !codeError && pinCode.length === inputIndex + 1,
                'blur-bottom-border': !codeError && pinCode.length !== inputIndex + 1
            }" />
                </div>
                <v-chip v-show="codeError" class="chip mt-4"> Неверный код </v-chip>
                <v-progress-circular v-if="isLoading" width="2" class="loading mx-auto text-center mt-4" color="active"
