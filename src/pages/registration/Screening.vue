<template>
    ...
    <div class="flex flex-row m-4 justify-between gap-2">
      ...
      <div v-else v-for="step in totalSteps" :key="step" class="progress-bar"
           :class="{ 'cursor-not-allowed': step > maxReachedStep }"
           @click="setPageStep(step)"
    ...
    <v-col>
      ...
      <div v-if="pageStep === 2">
        ...
        <div class="back w-full upload-wrapper mx-auto">
          <v-file-input @change="uploadImage" accept="image/*" class="input-file bg-black" />
          <img v-if="!user.pictureUrl" src="@/assets/img/regSteps/addProfilePic.svg" class="rounded-circle" />
          <img v-if="user.pictureUrl" :src="blobPic" class="rounded-circle img mx-auto" />
        </div>
        <UiButton @click="nextStep" class="mt-6" bgColor="blue">{{ user.pictureUrl ? 'Продолжить' : 'Выбрать аватар' }}</UiButton>
      ...
      <!-- Step 4 -->
      <div v-if="pageStep === 4">
        <UiPrompt>Если пользователи и владельцы проектов будут сразу понимать, какими навыками обладает собеседник - общаться станет удобнее.</UiPrompt>
  ...
  const user = ref({ firstName: '', country: '', city: '', pictureUrl: '' });
  const blobPic = ref('');
  ...
  const uploadImage = async (event: any) => {
    ...
    try {
      ...
      user.value.pictureUrl = URL.createObjectURL(file);
    } catch (error) {
